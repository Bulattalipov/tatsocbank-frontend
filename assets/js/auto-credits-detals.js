...
$(document).ready(function () {
  ...
  function transferPrimaryData (){
    ...
    let $range1 = $('.js-range-slider-auto-1'),
        $range3 = $('.js-range-slider-auto-3');

    console.log(autoDetals.min);

    $range1.attr("data-min", autoDetals.min);
    if(autoDetals.max) {
      $range1.attr("data-max", autoDetals.max);
    }

    // $range3.attr("data-min", autoDetals.feeMin);
    // if(autoDetals.max) {
    //   $range3.attr("data-max", autoDetals.feeMax);
    // }
  }
...
function auto($form) {
  ...
  let creditAmount = $('[name="credit-amount"]');
  ...
  // if ($.inArray(type, autoDetals.type) === -1)
  //         add = false;

  // if (autoDetals.min > amount || autoDetals.max < amount)
  //     add = false;

  // if (initial > amount)
  //     add = false;

  let percentFee = Number((initial / amount * 100).toFixed(2));
  // console.log(initial + ' -- ' + amount + ' -- ' + value.feeMax + ' -- ' + value.feeMax);
  if (autoDetals.feeMin > percentFee || autoDetals.feeMax < percentFee)
      add = false;
  else
  {
  ...
  if ($.inArray(term, autoDetals.terms) === -1)
      add = false;

  let filterProof = true;
  $.each(autoDetals.options, function (idOption, option) {
      if (proof === idOption) {
          if (option !== false) {
  ...
  // if (family1 === undefined && autoDetals.family1 !== '') {
  //     add = false;
  // }
  // if (family2 === undefined && autoDetals.family2 !== '') {
  //     add = false;
  // }

  if (salary !== undefined && salary === 'Y')
      oneResult.add += clearPercent(autoDetals.salary);

  if (lifeInsurance !== undefined && lifeInsurance === 'Y')
      oneResult.add += clearPercent(autoDetals.lifeInsurance);
  ...
  if (add) {
      ...
      if (autoDetals.delay) {
          defer = ((amount - initial) * autoDetals.delay[term] / 100).toFixed();
          constant = ((remainder - defer) / (term - 1)).toFixed(2);
      }

      let activeRate = 0;
      $.each(autoDetals.rates[term], function (fee, valueRate) {
          if (percentFee >= fee)
  ...
  if (creditAmountVal < 0)
      creditAmountVal = 0;
  creditAmount.text(number_format(creditAmountVal, 0, "", " "));
