...
$(document).ready(function () {
     ...
     * @param {Number} main - сумма кредита
     * @param {Number} time - срок кредита
     * @returns {{sum: number, percents: number, main: number}} - sum: итоговая сумма платежа, percents: итоговая сумма процентов, main: основной долг
     */
    ...
    function getResult() {
        ...
        const overPay = container.querySelector(".result3");

        monthPayment.textContent = numberWithSpaces(Math.round(result.average));
        rate.textContent = Math.round(result.rate * 100 * 100) / 100;
        overPay.textContent = numberWithSpaces(Math.round(result.percents));
    ...
    $(document).on('click change keyup keydown', '.js-d-update-range', function () {
        ...
        $input2.prop('value', numberWithSpaces(newFrom));
